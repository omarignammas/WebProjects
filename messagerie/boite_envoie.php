...
<header>
        <nav>
            <ul>
                ...
                <li><a href="boite_envoie.php">Messages Envoyees</a></li>
     ...
     <main>
        ...
        <div class="sec"><h2>Boite Envoyes</h2></div>
    <table border="1">
        <tr>
            <th>Destinataire</th>
            <th>Objet</th>
        ...
        <?php
        $req3 = "SELECT * FROM Messages WHERE IdExpediteur=$id ;";
        $res3 = mysqli_query($link,$req3);
        if (mysqli_num_rows($res3) > 0){
        while($data = mysqli_fetch_assoc($res3)){
            $id_des=$data['IdDestinataire'];
            $req = "SELECT * FROM `Utilisateurs` WHERE IdUtilisateur=$id_des;";
